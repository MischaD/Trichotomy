...
def process(rank, world_size, file_list, model, config, save_path):
    ...
    with torch.no_grad():
        for latents, indices, paths in pbar:
            ...
            latents = latents.to(f"cuda:{rank}")
            images = model.decode(latents)

            # Save each latent for every image in the batch
            for i, path in enumerate(paths):
                img_save_path = os.path.join(save_path, path[:-3])
                os.makedirs(os.path.dirname(img_save_path), exist_ok=True)
                Image.fromarray(images[i:i+1].permute(0, 2, 3, 1).cpu().numpy(), 'RGB').save(img_save_path)
